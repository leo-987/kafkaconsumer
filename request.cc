...
int GroupCoordinatorRequest::NumBytes()
{
	return Request::NumBytes() +		// head
...
int GroupProtocol::NumBytes()
{
	return 2 + assignment_strategy_.length() +
		   4 /* bytes */ + protocol_metadata_.NumBytes();
}
...
int GroupProtocol::Package(char **buf)
{
	...
	int protocol_metadata_size = htonl(protocol_metadata_.NumBytes());
...
int MetadataRequest::NumBytes()
{
	int size = Request::NumBytes();
	size += 4;
